...
const COPY = {
  ar: {
    ...
    subtitle: "أكمل بيانات بطاقتك في النموذج الآمن أدناه لإتمام الطلب.",
    loading: "جارٍ تجهيز صفحة الدفع...",
    ...
    failed: "تعذّر تجهيز صفحة الدفع",
    failedDesc:
      "لم نتمكن من تجهيز نموذج الدفع. لم يتم خصم أي مبلغ. يمكنك المحاولة مرة أخرى.",
    ...
    declinedDesc:
      "رفض البنك أو بوابة الدفع هذه العملية ولم يتم خصم أي مبلغ. يمكنك إعادة المحاولة ببطاقة أخرى أو اختيار طريقة دفع مختلفة.",
    declineReason: "سبب الرفض",
    retryPayment: "إعادة المحاولة بالدفع",
  },
  en: {
    title: "Card payment",
    subtitle: "Complete your card details in the secure form below to finish your order.",
    loading: "Preparing the payment page...",
    ...
    failed: "Could not prepare the payment page",
    failedDesc:
      "We couldn't prepare the payment form. No amount was charged. You can try again.",
    retry: "Try again",
    orderNo: "Order #",
    ...
    declinedDesc:
      "Your bank or the payment gateway declined this transaction and nothing was charged. You can try again with another card or choose a different payment method.",
...
export default function CheckoutPaymobPayPage() {
  ...
  // Resolve the iframe URL: use the one stashed by the checkout page if
  // present, otherwise ask the server for a fresh payment URL.
  useEffect(() => {
    if (!validOrder) {
      setStatus("failed");
      return;
    ...
    (async () => {
      ...
      try {
        url = sessionStorage.getItem(`paymob-checkout-${orderId}`);
      } catch {
        url = null;
      }
      if (!url) {
        try {
          ...
          const res = await fetch(`/api/store/orders/${orderId}/paymob-checkout`, {
            method: "POST",
            credentials: "include",
          ...
          if (res.ok && body.checkoutUrl) {
            url = String(body.checkoutUrl);
          } else if (res.status === 400 && typeof body.error === "string" && body.error.includes("already paid")) {
  ...
  const markPaid = useCallback(() => {
    if (!clearedRef.current) {
      clearedRef.current = true;
      try {
        sessionStorage.removeItem(`paymob-checkout-${orderId}`);
      } catch {
        // ignore
      }
      clear();
  ...
  const confirmPayment = useCallback(async (): Promise<boolean> => {
    try {
      ...
      if (res.ok && body.paymentStatus === "failed") {
        // Declined — swap the iframe for the decline screen right away.
        try {
          sessionStorage.removeItem(`paymob-checkout-${orderId}`);
        } catch {
          // ignore
  ...
  // Poll for payment confirmation while the iframe is shown. The Paymob
  // webhook usually flips the order first; this poll picks that up (and also
  // verifies directly with Paymob as a fallback).
  useEffect(() => {
  ...
  // Retry after a decline: drop any stale iframe URL and rerun the load
  // effect — the server reopens the failed order and issues a fresh iframe.
  ...
  return (
    <div className="min-h-screen bg-background" dir={isAr ? "rtl" : "ltr"}>
      ...
      <div className={status === "paying" ? "max-w-4xl mx-auto px-6 py-8" : "max-w-2xl mx-auto px-6 py-20"}>
        {status === "loading" && (
        ...
        {status === "paying" && checkoutUrl && (
          <div className="space-y-4">
            <div className="bg-card border border-border" data-testid="paymob-iframe-wrap">
              <iframe
                key={checkoutUrl}
                src={checkoutUrl}
            ...
            <div className="bg-card border border-border p-4 flex flex-col sm:flex-row items-center justify-between gap-3">
              ...
              <Button
                ...
                data-testid="button-paymob-confirm"
              >
                {verifying ? (
            ...
            <div className="bg-card border border-border p-4 flex flex-col sm:flex-row items-center justify-between gap-3">
              ...
              <Button
                ...
                data-testid="button-paymob-cancel"
              >
                {cancelling ? (
                  <Loader2 className="w-4 h-4 animate-spin" />
                ) : (
        ...
        {status === "declined" && (
          <div className="bg-card border border-border p-8 text-center" data-testid="paymob-declined">
        ...
        {status === "cancelled" && (
          <div className="bg-card border border-border p-8 text-center" data-testid="paymob-cancelled">
